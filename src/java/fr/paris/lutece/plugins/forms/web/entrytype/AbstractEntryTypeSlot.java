...
import static fr.paris.lutece.plugins.forms.util.FormsConstants.MESSAGE_ERROR_CRENEAUX;
import static fr.paris.lutece.plugins.forms.util.FormsConstants.MESSAGE_ERROR_CRENEAUX_IMPOSSIBLE;

public abstract class AbstractEntryTypeSlot extends EntryTypeService
{
    // PARAMETERS
    public static final String PARAMETER_SUFFIX_BEGIN       = "_debut";
    public static final String PARAMETER_SUFFIX_END         = "_fin";
    ...
    public GenericAttributeError getResponseData( Entry entry, HttpServletRequest request, List<Response> listResponse, Locale locale )
    {
        ...
        else if (StringUtils.isBlank(strBeginHour) || StringUtils.isBlank(strEndHour))
        {
            ...
            error.setErrorMessage( I18nService.getLocalizedString( MESSAGE_ERROR_CRENEAUX, locale ) );
            return error;
        }
        else if ( LocalTime.parse( strEndHour ).isBefore( LocalTime.parse( strBeginHour ) ) )
        {
            ...
            error.setMandatoryError( entry.isMandatory( ) );
            error.setTitleQuestion( entry.getTitle( ) );
            error.setErrorMessage( I18nService.getLocalizedString( MESSAGE_ERROR_CRENEAUX_IMPOSSIBLE, locale ) );
            return error;
        }
        else
        {
